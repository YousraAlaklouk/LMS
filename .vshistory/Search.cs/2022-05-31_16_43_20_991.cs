...
/*          var title = BCollection.Find({ "EmployeeName":});
            {
              */  var mongoBuilder = Builders<AddBook1>.Filter;
                var filter = mongoBuilder.Eq(y => y.TITLE, "full time");


                if (!string.IsNullOrEmpty(textBox1))
                {
                    textBox1 = "/.*" +textBox1 + ".*/i"; // this regex will search all the records which contains textToSearch and ignores case
                    filter = filter & mongoBuilder.Regex(y => y.Title, new BsonRegularExpression(textBox1));
                }
                //get the values only 
                //string booktitle = title.First().TITLE;

                DataBook.DataSource = title.ToList();
