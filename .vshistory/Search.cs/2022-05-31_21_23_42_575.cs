...
namespace LibraryManegement
{
    public partial class Search : Form
    {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            {
                string search2 = Convert.ToString(txtSearch.Text);

                var queryExpr2 = new BsonRegularExpression(new Regex(search2, RegexOptions.None));

                var filter = builder.Regex(a => a.RATE, queryExpr2); 
                var matchedDocuments = BCollection.FindSync(filter).ToList();
                DataBook.DataSource = matchedDocuments;
