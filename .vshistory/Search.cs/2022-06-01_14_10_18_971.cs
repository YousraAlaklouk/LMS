...
namespace LibraryManegement
{
    public partial class Search : Form
    {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            {
                ObjectId search3 = ObjectId.Parse(txtSearch.Text);
                Convert.ToString(search3);
                var queryExpr2 = new BsonRegularExpression(new Regex(Convert.ToString(search3), RegexOptions.None));

                var filter = builder.Regex(a => a.Id, queryExpr2);
            ...
            {
                var filtirDefinition = Builders<AddBook1>.Filter.Eq(a => a.RATE, Convert.ToInt32(txtSearch.Text));
            ...
            {
                var filtirDefinition = Builders<AddBook1>.Filter.Eq(a => a.QUANTITY, Convert.ToInt32(txtSearch.Text));
                var book = BCollection.Find(filtirDefinition).ToList();
                DataBook.DataSource = book;
            ...
            {
                var filtirDefinition = Builders<AddBook1>.Filter.Eq(a => a.PAGECOUNT, Convert.ToInt32(txtSearch.Text));
                var book = BCollection.Find(filtirDefinition).ToList();
                DataBook.DataSource = book;
