        ...
        private void btnAddBook_Click(object sender, EventArgs e)
        {
            if (ADD_OR_UPDATE == 0)
            {
                try
                {
                    ...
                    Book.TITLE = txtboxAddBookTitle.Text.Trim();
                    Book.AUTHOR_ID = cmbboxAuthorsName.SelectedValue.ToString();
                    Book.PUBLISHER_ID = cmbboxPublisherName.SelectedValue.ToString();
                    Book.CAT_ID = cmbboxCategory.SelectedValue.ToString();
                    ...
                    Book.EDITORS = txtEditor.Text.Trim();
                    Book.LANGUAGE = txtLang.Text.Trim();
                    ...
                    MessageBox.Show("Book Added Succefully","SUCCESS", MessageBoxButtons.OK , MessageBoxIcon.Information);
            ...
            else
            {
                try
                {
                    var updateDef = Builders<AddBook1>.Update
                                .Set(a => a.TITLE, txtboxAddBookTitle.Text.Trim())
                                .Set(a => a.AUTHOR_ID, cmbboxAuthorsName.SelectedValue.ToString())
                                .Set(a => a.PUBLISHER_ID, cmbboxPublisherName.SelectedValue.ToString())
                                .Set(a => a.CAT_ID, cmbboxCategory.SelectedValue.ToString())
                                .Set(a => a.ISBN_NO, txtboxAddBookIsbn.Text.Trim())
                                .Set(a => a.QUANTITY, Convert.ToInt32(txtboxQuantity.Text))
                                .Set(a => a.EDITION, txtEdition.Text.Trim())
                                .Set(a => a.EDITORS, txtEditor.Text.Trim())
                                .Set(a => a.LANGUAGE, txtLang.Text.Trim())
                                .Set(a => a.PAGECOUNT, Convert.ToInt32(txtCount.Text))
                                .Set(a => a.DATE, BookDate.Text.ToString())
                                .Set(a => a.RATE, bunifuRating1.Value);

                    BCollection.UpdateOne(s => s.Id == ObjectId.Parse(ID), updateDef);
                    MessageBox.Show("Book Updated Succefully", "SUCCESS", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    BooksForm.LoadBookData();
                }
                ...
                catch (Exception ex)
                {
                    ...
                    MessageBox.Show("While updating a book an Error catched" + ex.Message +" \n"+ ex.StackTrace, "Error Catched", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                finally
                {
