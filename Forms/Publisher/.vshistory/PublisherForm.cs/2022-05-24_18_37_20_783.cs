...
/* --------------------------Buttons-----------------------------------------------*/
        ...
        private void btnAddPublisher_Click(object sender, EventArgs e)
        {
            ...
            if (IsValid() <= 0)
            {
                try
                {
                    using (SqlConnection con = new SqlConnection(AppConnection.GetConnectionString()))
                    {
                        using (SqlCommand cmd = new SqlCommand("ADD_PUBLISHER", con))
                        {
                            ...
                            SuccForm.lblTaskCompleted.Text = "Publisher Added";
                            SuccForm.ShowDialog();
                        }
                    }
                }
                ...
                {
                    MessageBox.Show("While Adding a Publisher an Error catched \n" + ex.Message + "\n" + ex.StackTrace, "Catched an Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                finally
                {
                    ClearTextBoxes(this.Controls);
                    txtboxPublisherName.Focus();
                }
            }
        }
        //Delete Publisher
                ...
                if (checkPublisherBeforeDelete().Equals(true))
                {
                    ...
                    if (rslt == DialogResult.Yes)
                    {
                        try
                        {
                            using (SqlConnection con = new SqlConnection(AppConnection.GetConnectionString()))
                            {
                                using (SqlCommand cmd = new SqlCommand("DELETE_PUBLISHER", con))
                                {
                                    cmd.CommandType = CommandType.StoredProcedure;
