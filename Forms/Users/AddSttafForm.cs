...
{
    public partial class AddSttafForm : Form
    {
        public int ADD_TO_USER; //Hold the Id of the recent user added
   ...
   /*         if (ADD_OR_UPDATE == 0)
                    ...
                    try
                    {
                        ...
                        using (SqlConnection con = new SqlConnection(AppConnection.GetConnectionString()))
                        {
                            using (SqlCommand cmd = new SqlCommand("ADD_STAFF", con))
                            {
                                ...
                                SuccForm.lblTaskCompleted.Text = "Staff Added";
            ...
            else
            {
                try
                {
                    ...
                    using (SqlConnection con = new SqlConnection(AppConnection.GetConnectionString()))
                    {
                        using (SqlCommand cmd = new SqlCommand("UPDATE_STAFF", con))
                        {
                            ...
                            cmd.ExecuteNonQuery();
                            AddedSuccefulyDialog SuccForm = new AddedSuccefulyDialog();
                            SuccForm.lblTaskCompleted.Text = "Staff Updated ";
                            SuccForm.ShowDialog();
                            ClearTextBoxes(this.Controls);
                ...
                {
                    MessageBox.Show("While Updating a Staff  an Error catched \n" + ex.Message + "\n" + ex.StackTrace, "Catched an Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                finally
                {
                    txtboxName.Focus();
 ...
 /*           try
            {
                using (SqlConnection con = new SqlConnection(AppConnection.GetConnectionString()))
                {
                    using (SqlCommand cmd = new SqlCommand("COMBO_ROLES_USER", con))
                    {
                        ...
                        cmbboxUserRole.SelectedIndex = cmbboxUserRole.FindStringExact("Staff");
                    }
                }
            }
            catch (Exception ex)
